...
# --------------------------------------------------------------------------- #
def _resolve_name_collisions(plan: PagePlan) -> PagePlan:
    """A locator getter and a method cannot share a name.

    They share one namespace in a TypeScript class, so `private get
    searchResidents()` beside `async searchResidents(...)` is TS2300 duplicate
    identifier — and the method body's `this.searchResidents.fill(...)` would
    resolve to the method itself. Both names are perfectly reasonable in
    isolation, which is why the plan produced them.

    The method keeps the name: it is the public surface the steps bind to. The
    locator is a private implementation detail, so it takes the suffix.
    """
    method_names = {m.name for m in plan.methods}
    clashes = {prop for prop in plan.locators if prop in method_names}
    if not clashes:
        return plan

    renamed = {prop: _unique(f"{prop}Field", plan.locators) for prop in clashes}
    updated = replace(
        plan,
        locators={renamed.get(prop, prop): expr for prop, expr in plan.locators.items()},
        roles={renamed.get(prop, prop): role for prop, role in plan.roles.items()},
        methods=[
            replace(method, locators=[renamed.get(prop, prop) for prop in method.locators])
...
def _resolve_parameter_names(plan: PagePlan) -> PagePlan:
    """Every parameter of a method needs a distinct, usable name.

    A plan derived from `I fill the form with {string}` hands back the literal
    word "string" as the parameter name, once per placeholder. Rendered, that is

        async fillForm(string: string, string: string, string: string)

    which TypeScript rejects five times over with "Duplicate identifier
    'string'" — the parameter shadows the type in its own annotation, and then
    shadows itself. A whole page object failing to compile takes the suite with
    it, so this is not a cosmetic tidy-up.

    Renaming here rather than at the call sites is deliberate: the method body
    addresses parameters positionally, so fixing the list fixes the signature
    and every use of it at once.
    """
    methods = []
    for method in plan.methods:
        seen: set[str] = set()
        params: list[str] = []
        for index, raw in enumerate(method.params):
            name = safe_identifier(str(raw), fallback=f"value{index + 1}")
            if name in _TS_TYPE_NAMES or name in seen:
                name = f"value{index + 1}"
            while name in seen:                       # still taken: keep counting
                name = f"value{len(seen) + len(params) + 1}"
            seen.add(name)
            params.append(name)
        methods.append(replace(method, params=params) if params != method.params else method)
    return replace(plan, methods=methods)


#: Names that are legal identifiers and disastrous as parameter names, because
#: a parameter called `string` shadows the type in `string: string`.
_TS_TYPE_NAMES = frozenset(
    {
...
def render_page_object(plan: PagePlan, *, unverified: set[str] | None = None) -> str:
    ...
    plan = _resolve_name_collisions(plan)
    plan = _resolve_parameter_names(plan)
    lines: list[str] = [
        f"// {plan.class_name} - generated by QAgentic.",
    ]
    if plan.description:
...
def render_steps(
    ...
    *,
    ...
    # `noUnusedLocals` rejects, in a file the platform tells people to trust.
    used_pages = sorted({s.page for s in steps if s.page})
    keywords = [
        keyword
        for keyword in ("Given", "When", "Then")
        if any((s.keyword if s.keyword in ("Given", "When", "Then") else "Given") == keyword for s in steps)
    ] or ["Given"]

    lines: list[str] = [
        "// Generated by QAgentic - step definitions.",
        "// Steps call Page Object methods; locators never appear here.",
        f"import {{ {', '.join(keywords)} }} from '{bdd_import}';",
    ]
